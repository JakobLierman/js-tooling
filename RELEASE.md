...
- **Trigger**: Push to `develop` branch or manual trigger with "alpha" type
- **Version**: Creates alpha snapshot releases (e.g., `1.0.0` → `1.1.0-alpha.1`)
- **NPM Tag**: `alpha`
- **Git Tags**: Created for each package (e.g., `@jakoblierman/commitlint-config@1.1.0-alpha.1`)
- **Git Commits**: Version bumps and changelog updates committed automatically
- **Registry**: GitHub Packages
- **Changelog**: Generated by changesets from changeset files
- **Process**: Direct publish without PR (snapshot release)
...
### Beta/Alpha Releases (staging/develop branches)
...
- **Push to branches**: Runs tests on every push to `main`, `staging`, or `develop`
...
- Click "Run workflow"
- Choose the release type (production, beta, or alpha)
- Click "Run workflow"
...
This allows for a proper release pipeline where features flow through alpha → beta → production stages.
